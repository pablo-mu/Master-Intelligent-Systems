...
class PBVS_Actuator(VisualServo):
    def __init__(self,camera, lmbda = 0.05):
        self.camera = camera
        self.lmbda = 0.05
    
    def actuate(self, velocity):
        Td = velocity.interp1(self.lmbda)  # Assuming SE3.Delta can construct SE3 from velocity
        self.camera.pose @= Td
        
class PBVS_featurextractor(VisualServo):
...
class PBVS_Controller(VisualServo):
    def __init__(self, eterm=0, lmbda=0.05):
        self.eterm = eterm
        self.lmbda = lmbda
        self.pose_d = None

    def compute_motion(self, Te_C_G, pose_d):
...
class PBVS(VisualServo):
    def __init__(self, camera, eterm=0, lmbda=0.05,**kwargs):
        print("This is the personalized PBVS class, with the separates components.")
        super().__init__(camera, type="point", title="PBVS simulation", **kwargs)
        self.actuator = PBVS_Actuator(camera)
        self.featurextractor = PBVS_featurextractor(camera, self.P, self.pose_g)
        self.controller = PBVS_Controller(eterm, lmbda)
        
    def step(self,t):
        status = 0
        Te_C_G = self.featurextractor.compute_current_view()
        T_delta = self.controller.compute_motion(Te_C_G, self.pose_d)
        self.actuator.actuate(T_delta)
        hist = self._history()
        ...
        if np.linalg.norm(vel) < self.controller.eterm:
            status = 1
...
class IBVS(VisualServo):
    def __init__(
        ...
        **kwargs,
        ...
        print("This is the personalized IBVS class, with the separates components.")
