/*
Complete the file “mat_vec_col.c”, so that all the processes work together to compute 
a matrix-vector product. 
First, process 0 must distribute the matrix by block of columns and the vectors by block 
of rows. Then, the local computations have to be done to get a local solution which has 
to be added and redistributed. Finally, the solution must be stored in process 0. All 
