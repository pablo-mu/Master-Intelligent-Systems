    ...
    for test in tests:
        ...
        for i, imfile in enumerate(files):
            ...
            if test is "testHough":
                ...
                if bLecturerVersion:
                    p4e.displayLines(peaks_thetas, peaks_rhos, peaks_values) # exercise
                    plt.show(block=True)
...
def HOG(im, nbins=9):
    ...
    gradient_x = np.gradient(im, axis=1)
    gradient_y = np.gradient(im, axis=0)

    #Compute gradient magnitude and orientation
    #mag = np.sqrt(gradient_x**2 + gradient_y**2)
    ang = np.degrees(abs(np.arctan2(gradient_y, gradient_x)))
    # Create an array to store histograms
    bins = np.linspace(0, 180, nbins, endpoint=False)
    return np.ndarray.flatten(ang), bins


if __name__ == "__main__":
    #doTests()
    import numpy as np
    image = np.array(Image.open('./imgs-P4/lena.pgm'))
    # Compute the HOG with 9 bins
    hog_angles, bins = HOG(image, nbins=9)
    # Display the HOG features
    plt.hist(hog_angles, bins= bins, color='skyblue', edgecolor='black')
    plt.show()
