...
def display_magnitude_and_orientation(magnitude, angle):
    ...
    fig, ax = plt.subplots()
    plt.hist(magnitude[magnitude>0].flatten(), bins =14, range = (0, 15), label="LK, seq="+seq, fc=(0, 0, 1, 0.2))
    #bins = 30
...
if __name__ == "__main__":
    ...
    if not bSyntheticTransf:
        ...
        seq = 'corridor'
        seq = 'taxi'
        I1, I2 = get_real_sequence_image_pair(seq,20,21)
    else:
        seq = 'moon'
        seq = 'astronaut'
        seq = 'camera'
        ...
        translation = (0, 0) # (tx, ty) in pixel, positive or negative
    ...
    magnitude = np.sqrt(u**2 + v**2).flatten() # replace with your code
    angle = np.arctan2(v,u)
