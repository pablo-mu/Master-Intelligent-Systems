...
to select-winner
  ...
  let finishing-cfps []  ; list of the following cfps finished
      ; iterate over all the cfps.
      foreach initiator-cfps [
        id-whale-ticks -> let id-conv first id-whale-ticks
        ; checking if we have all the responses for that ContractNet
        if count-mailbox-id id-conv = num-boats - 1
        [
          set finishing-cfps lput id-whale-ticks finishing-cfps                   ; id of the finish initiate-cfps
          let responses-msgs get-lmsgs-key mailbox "conversation-id" id-conv      ; selecting all the messages with the same id
          if not empty? responses-msgs [
            set mailbox remove-lmsgs mailbox responses-msgs                       ; removing this messages from mailbox
            let propose-msgs get-lmsgs-key responses-msgs "performative" "propose" ;selecting propose messages
            ifelse not empty? propose-msgs[
              let winner-msg get-min-msg propose-msgs                             ; message with the less distance to the whale
              let winner get-value-msg winner-msg "sender"                        ; sender of that msg
              let accept-msg create-msg "accept-proposal"                         ; creating an accept-proposal to that boat
              set accept-msg set-value-msg accept-msg "content" last id-whale-ticks
              set accept-msg set-value-msg accept-msg "conversation-id" id-conv
              send-msg accept-msg winner
              let reject-msg create-msg "reject-proposal"                         ; rejecting the proposal to the other boats
              set reject-msg set-value-msg  reject-msg "conversation-id" id-conv
              send-msg reject-msg other boats with [self != winner]
          set succesful-cfps lput get-value-msg initiator-cfps id-conv succesful-cfps
          ]
          [
              ; rejcting proposal if no messages of propose
              let reject-msg create-msg "reject-proposal"
              set reject-msg set-value-msg  reject-msg "conversation-id" id-conv
              send-msg reject-msg other boats
              set failed-cfps lput last id-whale-ticks failed-cfps
          ]
      ]

        ]
      ]
  ;removing finished initiator-cfps
  set initiator-cfps remove-lmsgs initiator-cfps finishing-cfps
end
...
to revise-my-whales
  ...
  let finish-failed-cfps []
  foreach failed-cfps [
    x -> initiate-new-cfps x
    set finish-failed-cfps lput x finish-failed-cfps
  ]
  ; Deleting failed-cfps that have been initiated
  set failed-cfps remove-lmsgs failed-cfps finish-failed-cfps
end
...
to acting-as-follower
   ...
   ask boats[
    if not empty? follower-cfps[                  ;checking if there is some contract-net conversations to finish and finish in that case
      receive-responses-to-proposals-cfps
      ]
    follower-response
    ]
...
; first responses of the ContractNet
to follower-response
    let cfp-mails get-lmsgs-key mailbox "performative" "cfp"              ; selecting the messages that initiated the cfps
    if not empty? cfp-mails[
    set mailbox remove-lmsgs mailbox cfp-mails                            ; deleting from the mailbox to increase performance
  ]
    ; for each Contract_Net protocol we send a propose message if the whale indicated is under the surveillance of the boat sensor
    foreach cfp-mails [
      _x ->
      let whale-ticks get-value-msg _x "content"                         ; selecting the whale-ticks info
      let id-conv get-value-msg _x "conversation-id"                     ; Id of the contracNet
      set follower-cfps lput id-conv follower-cfps                       ; Setting the Id of the ContractNet into the follower-cfps list
      let sender get-value-msg _x "sender"
      ifelse distance first whale-ticks <= ratio-sensor[                 ; checking if the whale is under the surveillance of its sensor
        let propose-msg create-msg "propose"                             ; if it is we create a propose msg with the info of the distance.
        set propose-msg set-value-msg propose-msg "content" distance first whale-ticks
        set propose-msg set-value-msg propose-msg "conversation-id" id-conv
        send-msg propose-msg sender
      ]
      [
      ; if not, we send a reject message.
       let reject-msg create-msg "reject"
       set reject-msg set-value-msg reject-msg "conversation-id" id-conv
       send-msg reject-msg sender
      ]
    ]
end
...
to initiate-new-cfps [ _x ]
   ;; Code to complete
  let cfp-msg create-msg "cfp"
  set cfp-msg set-value-msg cfp-msg "conversation-id" increment-and-get-id
  set cfp-msg set-value-msg cfp-msg "content" _x
  send-msg cfp-msg other boats
...
to-report create-msg [value-of-performative]
  ...
  let newmsg list performative sender
  report newmsg
end

to-report set-value-msg [msg key value]
  let newkeyvalue list key value ; we create a list of key value pairs
  set msg lput newkeyvalue msg ; we add the list in msg
  report msg
end
...
;; Iterate over all the pairs
  foreach msg [
    ...
    [acl_msg] -> if (item 0 acl_msg) = key [report item 1 acl_msg] ; item 1
  ]
end

to send-msg [msg agentset]
  ;Iterate over each agent in the agentset
  ask agentset[
    set mailbox fput msg mailbox
  ]; update the agent's mailbox by adding the msg.
end

; method to count how many messages have the same conversation id.
to-report count-mailbox-id [_id]
  let messages-id get-lmsgs-key mailbox "conversation-id" _id
  report length messages-id
end
...
; method to get a list of messages with the given value for the given key.
to-report get-lmsgs-key [lmsgs key value]
  let filtered-msgs filter [msg -> get-value-msg msg key = value] lmsgs   ; filter the messages that meet the condition.
  ifelse empty? filtered-msgs[
    report []                                                             ; reporting an empty list if no msgs hold the condition.
  ]
  [
    report filtered-msgs
  ]
end

; method to remove a list of messages from a given list (usually mailbox).
to-report remove-lmsgs [lmsgs lmsgs-to-remove]
  report filter [msg -> not member? msg lmsgs-to-remove] lmsgs
end

;method to obtain the message of a list of messages with the minimum value of "content".
to-report get-min-msg [lmsgs]
  let min-content-values min map [ msg -> get-value-msg msg "content"  ] lmsgs ; save the minimum value of content
  report first get-lmsgs-key lmsgs "content" min-content-values ; reports the first message with the minimum value
end
